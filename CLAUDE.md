...
- [CRITICAL] All Node.js scripts and SKILL.md instructions must be compatible with Linux, macOS, and Windows: use `os.homedir()` instead of `process.env.HOME`, avoid shell-only constructs (HEREDOC, `[ -z ]`, etc.) in cross-platform paths, and guard platform-specific test logic with `process.platform` checks
