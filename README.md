...
| `codefree`     | 提供 `/codefree:*` 命令、专用 subagent 和两个内部不可直接调用的 skills，将编码任务委托给 codefree CLI   | [README.md](plugins/codefree/README.md)    |
...
`git-commit` 与 `daily-report` 采用用户显式调用，避免模型在未确认时自动执行提交或日报流程。`web-fetch` 可由模型在需要原始 Markdown、回退链或代理时选择。`codefree` 提供命令与专用 subagent；若希望用户提到“研发云”时自动路由，请在用户级或项目级 `CLAUDE.md` 中配置该编排策略。
